...
function presetForm(preset = {}) {
  ...
  return `<form class="preset-form" data-preset-form data-id="${id}">
    ...
    <div class="field preset-actions"><button class="secondary" type="submit">保存</button><button class="primary" type="submit" data-activate="1">启用</button></div>
  </form>`;
}
  ...
  const body = `<main class="page">
    ...
    <section class="scroll">
      <div class="settings-group"><div class="group-label">预设</div>
        ...
        ${store.addingPreset ? presetForm({ name: "新预设", format: "openai", ...(store.newPresetDraft || {}) }) : `<button class="add-btn" data-action="add-preset">${icon("plus")} 添加新预设</button>`}
      </div>
    </section>
  </main>`;
  return phone({ activeTab: "set", hideTab: true, body });
